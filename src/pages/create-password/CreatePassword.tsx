...
import { useFinishCreatePasswordMutation } from '@/services/userService';
...
const CreatePassword = () => {
  ...
  useEffect(() => {
    if (hasValidKey && key && key.trim() !== '') {
      ...
      const url = `${baseUrl}/api/account/create-password/validate?key=${encodeURIComponent(key)}`;
  ...
  const [finishCreatePassword, { isLoading }] = useFinishCreatePasswordMutation();
  ...
  const handleSubmit = async () => {
    ...
    try {
      await finishCreatePassword({ key, newPassword: formValue.newPassword }).unwrap();
  ...
  if (!key || !hasValidKey) {
    return (
      <Panel className="panel" style={{ backgroundImage: `url(${Error403Background})` }}>
        <ErrorPage code={403}>
