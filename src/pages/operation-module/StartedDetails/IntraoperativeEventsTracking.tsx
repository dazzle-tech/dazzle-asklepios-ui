...
import {
  ...
  newApOperationStaff,
  newApOperationSurgicalPreparationIncision
...
const IntraoperativeEventsTracking = ({ operation, patient, encounter, editable }) => {
  ...
  const { data: intraoperativeData } = useGetIntraoperativeEventsByOperationKeyQuery(
    operation?.key,
    {
      skip: !operation?.key,
  ...
  const { data: surgical, refetch } = useGetLatestSurgicalPreparationByOperationKeyQuery(
    operation?.key
  );
  ...
  const selectedNames = useMemo(() => {
    return (
      operations?.object?.filter(item => selectedKeys.includes(item.key)).map(item => item.name) ??
      []
    );
  ...
  useEffect(() => {
    if (intraoperativeData) {
      setIntraoperative({
        ...
        firstCountTime: intraoperativeData.object?.firstCountTime
          ? new Date(intraoperativeData.object.firstCountTime)
          : null,
        secondCountTime: intraoperativeData.object?.secondCountTime
          ? new Date(intraoperativeData.object.secondCountTime)
          : null,
        skinClosureTime: intraoperativeData.object?.skinClosureTime
          ? new Date(intraoperativeData.object.skinClosureTime)
          : null,
        surgeryEndTime: intraoperativeData.object?.surgeryEndTime
          ? new Date(intraoperativeData.object.surgeryEndTime)
          : null
      });

      setTag(
        intraoperativeData.object?.specimensTaken
          ? intraoperativeData.object?.specimensTaken.split(',')
          : []
      );
      setSelectedKeys(
        intraoperativeData.object?.actualOperationPerformed
          ? intraoperativeData.object?.actualOperationPerformed.split(',')
          : []
      );
      setInstruc(
        intraoperativeData.object?.surgicalComplications
          ? intraoperativeData.object?.surgicalComplications
          : null
      );
  ...
  useEffect(() => {
    if (surgical?.object?.key != null) {
      setSurgicalP({
        ...surgical.object,
        timeOfIncision: surgical.object.timeOfIncision
          ? new Date(surgical.object.timeOfIncision)
          : null
      });
    } else {
              ...
              <div className="container-form">
                ...
                <Row>
                  ...
                  >
                    ...
                    <Col md={8}>
                      <Radio value="Escalated">Escalated</Radio>
                    </Col>
                    <Col md={8}>
          ...
          <Row>
            <Col md={24}>
              <div className="container-form">
                ...
                {intraoperative.countDiscrepancy && (
                  <Row>
                    <Col md={24}>
                      <MyInput
                        width="100%"
