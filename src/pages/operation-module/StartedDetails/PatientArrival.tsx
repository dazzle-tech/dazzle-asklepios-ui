...
const PatientArrival = ({
  ...
  useEffect(() => {
    if (checklists?.object?.key !== null) {
      setCheckList({
        ...checklists?.object,
        confirmTime: checklists?.object?.confirmTime
          ? new Date(checklists?.object?.confirmTime)
          : null
      });
    } else {
  ...
  const handleConfirm = async () => {
    try {
      await saveCheckList({
        ...checkList,
        isConfirm: true,
        confirmTime: new Date(checkList.confirmTime).getTime()
      }).unwrap();
  ...
  const handleSave = async () => {
    try {
      await save({
        ...
        dateTime: new Date(arraivel?.dateTime).getTime()
      }).unwrap();
          ...
          <Col md={24}>
            <div className="container-form">
              ...
              >
                <Col md={3}>
                  <br />
                  <MyButton onClick={() => setOpenCheckList(true)}>
                    <Translate>Pre-Op Checklist</Translate>
                  </MyButton>
                </Col>
                <Col md={6}>
                  <MyInput
                ...
                <Col md={3}>
                  ...
                  {!checkList?.isConfirm ? (
                    ...
                    <MyButton appearance="ghost">Confirmed</MyButton>
                  )}
                </Col>
              </Row>
            </div>
