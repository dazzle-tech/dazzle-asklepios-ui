...
import { useGetDiagnosticOrderTestQuery, useGetOrderTestNotesByTestIdQuery, useSaveDiagnosticOrderTestNotesMutation } from "@/services/encounterService";
import { useGetDiagnosticsTestRadiologyListQuery } from "@/services/setupService";
import { newApDiagnosticOrderTests, newApDiagnosticOrderTestsNotes, newApDiagnosticOrderTestsRadReport } from "@/types/model-types-constructor";
...
console.log("tests",testsList)
    ...
    const testColumns = [
        {
            ...
            flexGrow: 1,
            render: (rowData: any) => {
                const cat = radiologyList?.object?.find(
    ...
    return (<>
        ...
        <PatientArrivalModal open={openArrivalModal} setOpen={setOpenArrivalModal}
            test={test} setTest={setTest} saveTest={saveTest} fetchTest={fetchTest} />
    </>)
}
