...
import { CountryDistrict, DistrictCommunity } from '@/types/model-types-new';
...
import CommunityAreaSubPanel from '../community-area/CommunityAreaSubPanel';
...
interface Props {
  ...
  setShowChild: (v: boolean) => void;
  setChildContent: (v: JSX.Element) => void;
  setChildTitle: (v: string) => void;
}

const DistrictCommunityChildPanel: React.FC<Props> = ({
  district,
  setShowChild,
  setChildContent,
  setChildTitle
  ...
  const [community, setCommunity] = useState<DistrictCommunity>({
    id: undefined,
  ...
  const [filteredData, setFilteredData] = useState<DistrictCommunity[]>([]);
  ...
  const { data, isFetching, refetch } = useGetCommunitiesByDistrictQuery(
    {
      districtId,
  ...
  const filterFields = [{ label: 'Community Name', value: 'name' }];
  ...
  const handleSave = async () => {
    ...
    const payload: DistrictCommunity = {
      ...community,
      districtId,
    ...
    try {
      if (isUpdate) {
        ...
        if (isFiltered) {
          ...
          if (matchesFilter) {
            ...
            dispatch(
              notify({
                msg: 'Community added but does not match current filter',
                sev: 'info'
              })
            );
          }
      ...
      const keyMap: Record<string, string> = {
        ...
        'district.required': 'District id is required.',
        'name.required': 'Community name is required.',
        'unique.community.name': 'A community with the same name already exists for this district.',
        'db.constraint': 'Database constraint violation while saving community.',
  ...
  const handleToggleActive = async () => {
    ...
    try {
      const updated = await toggleCommunityActive({ id: community.id }).unwrap();

      dispatch(
        notify({
          msg: updated.isActive ? 'Community Reactivated' : 'Community Deactivated',
          sev: 'success'
        })
      );
      ...
      dispatch(
        notify({
          msg: 'Failed to change community status',
          sev: 'error'
        })
      );
    }
  ...
  const handleOpenAreas = (row: DistrictCommunity) => {
    setCommunity(row);
    setChildTitle(`${row.name} Areas`);
    setChildContent(<CommunityAreaSubPanel community={row} />);
    setShowChild(true);
  };

  const iconsForActions = (r: DistrictCommunity) => (
  ...
  const columns = [
    { key: 'name', title: 'Community Name', flexGrow: 4 },
    {
      key: 'isActive',
    ...
    { key: 'icons', title: '', flexGrow: 2, render: (r: DistrictCommunity) => iconsForActions(r) }
  ];
  ...
  return (
    <div style={{ padding: 12 }}>
      ...
      <MyTable
        ...
        loading={isFetching}
        columns={columns}
        rowClassName={r => (r.id === community.id ? 'selected-row' : '')}
        filters={filters()}
        onRowClick={r => setCommunity(r as DistrictCommunity)}
...
export default DistrictCommunityChildPanel;
