...
const NphiesPayerModal: React.FC<NphiesPayerModalProps> = ({
  ...
  const tpaOptions = useMemo(() => {
    const byId = new Map<number, NonNullable<ReturnType<typeof normalizeTpaDefinition>>>();
    [...unwrapList(tpaResponse), ...(payer.tpas ?? [])].forEach(item => {
      const tpa = normalizeTpaDefinition(item);
      if (!tpa) {
        return;
      }
      const existing = byId.get(tpa.id);
      byId.set(tpa.id, {
        id: tpa.id,
        tpaCode: tpa.tpaCode || existing?.tpaCode || '',
        name: tpa.name || existing?.name || '',
        isActive: !tpa.tpaCode && !tpa.name ? existing?.isActive ?? tpa.isActive : tpa.isActive
      });
    });
    const linkedIds = new Set(
      (payer.tpaIds ?? []).filter((id): id is number => Number.isFinite(Number(id))).map(Number)
    );
    return [...byId.values()]
      .filter(tpa => tpa.isActive !== false || linkedIds.has(tpa.id))
      .map(tpa => ({
        value: tpa.id,
