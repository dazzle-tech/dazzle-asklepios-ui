...
// src/pages/accounting/Receipt.tsx
...
const Receipt: React.FC<ReceiptProps> = ({ patient }) => {
  ...
  const { data: invoicePage } = useGetInvoicesQuery(
    {
      page: 0,
      size: 50,
      sort: 'id,desc',
  ...
  const { data: itemsPage, isLoading } = useGetInvoiceItemsQuery(
    {
      page: 0,
      size: 500,
      sort: 'id,asc',
  ...
  const iconsForActions = () => (
    <div className="container-of-icons">
      <Printer size={22} className="icons-style" />
    </div>
  );
