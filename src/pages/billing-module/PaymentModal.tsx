...
const PaymentModal: React.FC<PaymentModalProps> = ({
  ...
  const [createPayment, { isLoading: isSavingPayment }] =
    useCreatePaymentMutation();
  const [createAllocation, { isLoading: isSavingAllocation }] =
    useCreatePaymentAllocationMutation();
    ...
    try {
      ...
      toaster.push(
        <Message showIcon type="error" closable>
          Failed to save payment: {e?.data?.message || e?.message || 'Unknown error'}
        </Message>,
  ...
  const conjureFormContent = () => (
    <Form fluid>
      <MyInput
        ...
        fieldType="number"
        record={{ amount }}
        setRecord={(r: any) => setAmount(r.amount)}
        required
        width="100%"
      />
      <MyInput
        ...
        record={{ method }}
        setRecord={(r: any) => setMethod(r.method)}
        width="100%"
      />
      <MyInput
        ...
        fieldType="text"
        record={{ reference }}
        setRecord={(r: any) => setReference(r.reference)}
        width="100%"
      />
      <MyInput
        fieldLabel="Notes"
        ...
        fieldType="textarea"
        record={{ notes }}
        setRecord={(r: any) => setNotes(r.notes)}
        width="100%"
      />
    </Form>
  );
  ...
  return (
    <MyModal
      ...
      size="sm"
      bodyheight="50vh"
      actionButtonLoading={isSavingPayment || isSavingAllocation}
    />
  );
};
