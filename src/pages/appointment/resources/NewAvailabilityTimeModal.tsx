...
//   const [fromTime, setFromTime] = useState({from: null});
//   const [toTime, setToTime] = useState({to: null});
//   const [timeSlices, setTimeSlices] = useState({});
//   const [sliceDuration, setSliceDuration] = useState({sliceDuration: SLICE_DURATION_MINUTES});
...
//   const {
//     data: facilityListResponse } = useGetAllFacilitiesQuery({});
...
//       const facilityFromList = facilityListResponse.find(
//         f => f.id?.toString() === resourceFacilityKey
//       );
//       if (facilityFromList) {
...
//   const { data: resourcesWithAvailabilityResponse, refetch: refetchAvailability } =
//     useGetResourcesWithAvailabilityQuery(
//       {
//         ...initialListRequest,
//         filters: resourceKey
//           ? [{ fieldName: 'resource_key', operator: 'match', value: resourceKey }]
//           : []
//       },
...
//   useEffect(() => {
//     if (
//       resourcesWithAvailabilityResponse?.object?.length > 0 &&
...
//           const facilityFromList = facilityListResponse.find(
//             f => f.id?.toString() === objItem.facilityKey
//           );
//           if (facilityFromList) {
//             setFacility(facilityFromList);
//           } else {
...
//         setFromTime({from: minutesToDisplayDate(globalStartMinutes)});
//         setToTime({to: minutesToDisplayDate(globalEndMinutes)});
//       }
...
//     const currentSliceStart = new Date(referenceDate);
//     currentSliceStart.setHours(start.getHours(), start.getMinutes(), 0, 0);

//     const endOfInterval = new Date(referenceDate);
...
//     if (!fromTime?.from || !toTime.to || selectedDays.length === 0) return;
//     const generated = {};
//     // originalSlices will contain objects with startTimeMinutes and endTimeMinutes (numbers)
//     const originalSlices = generateSlices(fromTime?.from, toTime.to, sliceDuration.sliceDuration);
...
//         errorMessage =
//           err.data.message || err.data.msg || err.data.detail || err.data.title || errorMessage;
...
//     if (slices.length === 0) {
//       const baseTime = fromTime['from'] ? new Date(fromTime['from']) : new Date();
//       baseTime.setHours(9, 0, 0, 0); // Default start if no fromTime
//       potentialNewFromTime = new Date(baseTime.getTime() - sliceDuration['sliceDuration'] * 60000);
...
//       potentialNewFromTime = new Date(firstSlice.from.getTime() - sliceDuration['sliceDuration'] * 60000);
...
//     if (slices.length === 0) {
//       const baseTime = toTime.to ? new Date(toTime.to) : new Date();
//       baseTime.setHours(9, 0, 0, 0);
//       potentialNewToTime = new Date(baseTime.getTime() + sliceDuration['sliceDuration'] * 60000);
...
//       potentialNewToTime = new Date(lastSlice.to.getTime() + sliceDuration['sliceDuration'] * 60000);
...
//         const baseTime = fromTime['from'] ? new Date(fromTime['from']) : new Date();
//         baseTime.setHours(9, 0, 0, 0);
//         newFrom = new Date(baseTime.getTime() - sliceDuration['sliceDuration'] * 60000);
//         newTo = new Date(baseTime);
//       } else {
//         const firstSlice = slices[0];
//         newFrom = new Date(firstSlice.from.getTime() - sliceDuration['sliceDuration'] * 60000);
...
//       if (!canAddSliceRight(day, slices, sliceDuration, toTime.to)) {
//         return prev;
//       }
...
//         newTo = new Date(newFrom.getTime() + sliceDuration['sliceDuration'] * 60000);
//       } else {
//         const lastSlice = slices[slices.length - 1];
...
//     setFacility(null);
//     setFromTime({from: null});
//     setToTime({to: null});
//     setSliceDuration({sliceDuration: SLICE_DURATION_MINUTES});
//     setSelectedDays([]);
//     setTimeSlices({});
//     }
//   },[open]);
...
//         <CheckboxGroup
//           value={selectedDays}
//           onChange={value => setSelectedDays(value as DayValue[])}
//         >
...
//       <MyButton appearance="ghost" onClick={handleGenerate} style={{ marginTop: '10px' }}>
//         {Object.values(timeSlices).some(
//           daySlices => Array.isArray(daySlices) && daySlices.length > 0
//         )
//           ? 'Regenerate slices'
//           : 'Generate slices'}
//       </MyButton>
...
//                 const fromStr =
//                   displayFrom instanceof Date && !isNaN(displayFrom.getTime())
//                     ? displayFrom.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
//                     : 'Invalid Date';

//                 const toStr =
//                   displayTo instanceof Date && !isNaN(displayTo.getTime())
