...
const ExternalLabAction = ({ rowData, onSuccess }: Props) => {
  ...
  const [externalTestState, setExternalTestState] = useState<ExternalTestState>({
    id: null,
    testId: orderTestId,
    facilityName: '',
    reason: ''
  ...
  const { data: externalTest, isFetching } = useGetExternalTestByTestIdQuery(
    orderTestId ?? skipToken,
    {
      refetchOnMountOrArgChange: true
    }
  );

  const [createExternalTest, { isLoading }] = useCreateExternalTestMutation();

  const { data: facilityListResponse, isFetching: isFacilitiesFetching } = useGetAllFacilitiesQuery(
    {
      page: 0,
      size: 100
    }
  ...
  const iconColor = isFetching ? '#999' : isAlreadyExternal ? '#1675e0' : 'var(--primary-gray)';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      dispatch(
        notify({
          msg: e?.data?.message || e?.data?.detail || 'Send to external lab failed',
          sev: 'error'
        })
      );
    }
