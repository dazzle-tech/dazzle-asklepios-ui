...
const renderMarker = (marker?: string) => {
  switch (marker) {
    ...
    case 'CRITICAL_UPPER':
      return (
        <HStack spacing={6} style={{ color: '#d32f2f' }}>
...
const ReviewResults = forwardRef<any, any>(
  ({ loading, setTest, setPatient, setEncounter }, ref) => {
    ...
    const handleReviewSelected = async () => {
      ...
      try {
        await bulkToggleReviewDiagnosticOrderTestResult({
          ids: selectedRows
        }).unwrap();
        ...
        dispatch(
          notify({
            msg: 'Failed to review selected results',
            sev: 'error'
          })
        );
      }
    ...
    const allRowIds = useMemo(
      () => normalizedResults.map(row => row.id),
      [normalizedResults]
    );
