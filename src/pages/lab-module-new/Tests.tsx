...
const Tests = forwardRef<unknown, Props>(
  ({ order, test, setTest, samplesList, resultFetch,fetchAllTests ,fecthSample }, ref) => {
    ...
    // Get lab details for the selected test
    const labDetail = laboratoryList?.object?.find(item => item.testKey === test?.testKey);

    // Fetch normal ranges for the selected test using new service (for non-profile tests)
    const { data: normalRangesData, refetch: refetchNormalRanges } = useGetDiagnosticTestNormalRangesByTestIdQuery(
      {
        testId: test?.testKey ? Number(test.testKey) : 0,
        page: 0,
        size: 100
      },
      {
        skip: !test?.testKey || labDetail?.isProfile
      }
    );

    // Fetch normal ranges for profile tests if applicable
    const { data: profileNormalRangesData, refetch: refetchProfileNormalRanges } = useGetDiagnosticTestNormalRangesByProfileTestIdQuery(
      {
        profileTestId: labDetail?.profileTestKey ? Number(labDetail.profileTestKey) : 0,
        page: 0,
        size: 100
      },
      {
        skip: !labDetail?.profileTestKey || !labDetail?.isProfile
      }
    );

    // Combine normal ranges data (use profile ranges if it's a profile test, otherwise use test ranges)
    const availableNormalRanges = labDetail?.isProfile 
      ? profileNormalRangesData?.data || []
      : normalRangesData?.data || [];

    // Log normal ranges for debugging (can be removed in production)
    useEffect(() => {
      if (availableNormalRanges.length > 0) {
        console.log('Normal ranges loaded:', availableNormalRanges);
      }
    }, [availableNormalRanges]);
    ...
    useEffect(() => {
      ...
      // Refetch normal ranges when test changes
      if (test?.testKey) {
        const currentLabDetail = laboratoryList?.object?.find(item => item.testKey === test?.testKey);
        if (currentLabDetail?.isProfile && currentLabDetail?.profileTestKey) {
          refetchProfileNormalRanges();
        } else if (currentLabDetail && !currentLabDetail?.isProfile) {
          refetchNormalRanges();
        }
      }
    }, [test, laboratoryList]);
