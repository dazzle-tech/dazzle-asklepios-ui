...
import { useGetNursingCareByOperationQuery, useSaveNursingCareMutation } from "@/services/RecoveryService";
...
const NursingCare = ({operation}) => {
  ...
  useEffect(() => {
    setCare({ ...care,
       woundDressingStatus: woundDressingStatus })
  }, [woundDressingStatus]);

 useEffect(() => {
    if (careData?.object) {
      setCare(careData.object); 
      setIvFluidsGiven(careData.object.ivFluidsGiven ? careData.object.ivFluidsGiven.split(',') : []);
      setAnalgesicsGiven(careData.object.analgesicsGiven ? careData.object.analgesicsGiven.split(',') : []);
      setDrainsAndTubes(careData.object.drainsTubes? careData.object.drainsTubes.split(',') : []);
      setComplicationsObserved(careData.object.complicationsObserved ? careData.object.complicationsObserved.split(',') : []);
  ...
  return (
    <div className="container-form">
      ...
      <Row className="container-of-radio-recovery">
        ...
        <Col md={16}>
          <RadioGroup name="woundDressingStatus"
            inline
