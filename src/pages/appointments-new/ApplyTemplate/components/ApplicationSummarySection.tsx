...
const ApplicationSummarySection: React.FC<ApplicationSummarySectionProps> = ({
  ...
  const holidaySummaryLine =
    excludeHolidays || includeAsException
      ? excludeHolidays
        ...
        : exceptionCount === 0
          ? "No organization holidays in the selected range"
          : `${exceptionCount} organization holiday${
              exceptionCount === 1 ? "" : "s"
            } included as exceptions`
      : null;

  /**
   * This count comes from the same list that will be sent to apply/generation logic.
   * Expected DTO field:
   * policyAssignmentIds: number[]
   */
  const policyAssignmentIds = React.useMemo(() => {
    return ((dto as any)?.policyAssignmentIds ?? [])
      .map((id: any) => Number(id))
      .filter((id: number) => Number.isFinite(id) && id > 0);
  }, [(dto as any)?.policyAssignmentIds]);

  const policiesToApplyCount = policyAssignmentIds.length;

  const policiesSummaryLine =
    policiesToApplyCount === 0
      ? "No policies selected"
      : `${policiesToApplyCount} ${
          policiesToApplyCount === 1 ? "policy" : "policies"
        } will be applied`;
  ...
  const configCardClass = isDark
    ? "border-[#3d3d3d] bg-[var(--extra-dark-black)]"
      ...
      <div className="space-y-5">
        ...
        <div className={`rounded-2xl px-4 py-4 ${totalSlotsCardClass}`}>
          <div className="flex items-center justify-between gap-3">
            <span className={`text-sm font-semibold ${totalSlotsLabelClass}`}>
              Total slots
            </span>

            <Pill className={totalSlotsValueClass}>
              {totalSlotsToBeCreated}{" "}
              {totalSlotsToBeCreated === 1 ? "appointment" : "appointments"}
            </Pill>
          </div>
        ...
        <div className={`rounded-2xl p-4 ${configCardClass}`}>
          <div className={`text-sm font-semibold ${configTitleClass}`}>
            Configuration
          </div>
        ...
        <div className={`rounded-2xl p-4 text-sm ${infoCardClass} ${infoTextClass}`}>
          <div className="flex items-start gap-3">
            ...
            <p>
              <span className="font-semibold">Next step:</span> Free appointments will
              be created and ready for patient booking.
            </p>
          </div>
        </div>
      </div>
