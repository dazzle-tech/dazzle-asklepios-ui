...
import { useLazyGetPayorPlansByPayorQuery } from '@/services/setup/payer/PayorPlanService';
...
const InsuranceTab: React.FC<InsuranceTabProps> = ({ localPatient }) => {
  ...
  const [selectedInsurance, setSelectedInsurance] =
    useState<ApPatientInsurance | null>();
  const [InsuranceModalOpen, setInsuranceModalOpen] = useState(false);
  const [specificCoverageModalOpen, setSpecificCoverageModalOpen] =
    useState(false);
  ...
  const { data: payorListResponse, isFetching: payorFetching } =
  ...
  const [triggerGetPlans] = useLazyGetPayorPlansByPayorQuery();
  ...
  const handleDeleteInsurance = () => {
    ...
    deleteInsurance({ key: selectedInsurance.key }).then(() => {
      ...
      dispatch(
        notify({ msg: 'Insurance Deleted Successfully', sev: 'success' })
      );
      setSelectedInsurance(null);
  ...
  const columns = [
    {
      ...
      flexGrow: 4,
      render: (rowData: any) => {
        const payorId = Number(
          rowData?.insuranceProviderLkey ?? rowData?.insuranceProviderId
        );

        const payorName = conjureValueBasedOnIDFromList(
          payorsList,
          payorId,
          'name'
        );
    ...
    },
    {
      ...
      render: (rowData: any) => {
        ...
        const planId = Number(
          rowData?.insurancePlanTypeLkey ?? rowData?.insurancePlanTypeId
        );
        ...
        return (
          <span>
            {conjureValueBasedOnIDFromList(plansList, planId, 'name')}
          </span>
        );
      },
    },
