...
const OneGraphTab = ({ patient: patientProp }) => {
  ...
  const { data: weightData = [] } = useGetWeightListByPatientBetweenDatesQuery(
    { patientId, from: fromIso, to: toIso },
    { skip: !patientId }
  );

  const { data: heightData = [] } = useGetHeightListByPatientBetweenDatesQuery(
    { patientId, from: fromIso, to: toIso },
    { skip: !patientId }
  );

  const { data: tempData = [] } = useGetTemperatureListByPatientBetweenDatesQuery(
    { patientId, from: fromIso, to: toIso },
    { skip: !patientId }
  );

  const { data: pulseData = [] } = useGetPulseRateListByPatientBetweenDatesQuery(
    { patientId, from: fromIso, to: toIso },
    { skip: !patientId }
  );

  const { data: respData = [] } = useGetRespiratoryRateListByPatientBetweenDatesQuery(
    { patientId, from: fromIso, to: toIso },
    { skip: !patientId }
  );

  const { data: oxyData = [] } = useGetOxygenSaturationListByPatientBetweenDatesQuery(
    { patientId, from: fromIso, to: toIso },
    { skip: !patientId }
  );

  const { data: bpData = [] } = useGetBloodPressureListByPatientBetweenDatesQuery(
    { patientId, from: fromIso, to: toIso },
    { skip: !patientId }
  );
