...
const StartTriage = ({ patient, encounter, sourcePage }) => {
  ...
  useEffect(() => {
    ...
    const YES_KEY_LOCAL = "1476229927081534";
    ...
    if (
      emergencyTriage.lifeSavingLkey === YES_KEY_LOCAL ||
      emergencyTriage.unresponsiveLkey === YES_KEY_LOCAL
    ) {
      newLevel = "6859764100147954"; // critical
    } else if (
      emergencyTriage.highRiskLkey === YES_KEY_LOCAL ||
      ...
      const count = selectedServices.filter(value => value === YES_KEY_LOCAL).length;
 ...
 disableByField='isValid'
            ...
            {emergencyTriage?.emergencyLevelLkey && (
              <MyBadgeStatus
                ...
                contant={
                  selectedEmergencyLevel?.lovDisplayVale ??
                  emergencyTriage?.emergencyLevelLkey
                }
              />
            )}
