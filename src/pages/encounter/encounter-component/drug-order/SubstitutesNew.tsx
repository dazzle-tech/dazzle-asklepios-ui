...
import { useGetBrandMedicationSubstitutesByBrandQuery } from "@/services/setup/brandmedication/BrandMedicationSubstituteService";
const SubstituesNew = ({ open, setOpen, selectedGeneric, setSelectedGeneric }) => {
    const { data: lisOfLinkedBrand } = useGetBrandMedicationSubstitutesByBrandQuery(selectedGeneric?.id, { skip: selectedGeneric?.id == null });
    ...
    const totalCount = lisOfLinkedBrand?.length ?? 0;
    const paginatedData = lisOfLinkedBrand?.slice(
