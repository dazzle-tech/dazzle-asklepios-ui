...
const AddAudiometryPuretone = ({
  ...
  const [audiometryPuretone, setAudiometryPuretone] =
    useState<ApAudiometryPuretone>(audiometryPuretoneObject);
  const [saveAudiometryPureton] = useSaveAudiometryPuretoneMutation();
  const [isDisabledField, setIsDisabledField] = useState(false);
  const [isEncounterAudiometryPuretonStatusClose, setIsEncounterAudiometryPuretonStatusClose] =
    useState(false);
  ...
  const { data: hearingLossTypeLovQueryResponse } =
    useGetLovValuesByCodeQuery('HEARING_LOSS_TYPES');
  ...
  const handleSave = async () => {
    ...
    try {
      if (audiometryPuretone.key === undefined) {
        ...
        dispatch(
          notify({ msg: 'Patient Audiometry Pureton Updated Successfully', sev: 'success' })
        );
      }
      await refetch();
      handleClearField();
