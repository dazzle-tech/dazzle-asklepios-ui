...
const OptometricExamTabs = ({
  ...
  const visualAcuityColumns = [
    ...
    },
    {
      ...
      render: (row: any) =>
        row?.medicalHistoryLvalue ? row.medicalHistoryLvalue.lovDisplayVale : row.medicalHistoryLkey
    },
    {
      key: 'performedWith',
      title: 'TEST PERFORMED WITH',
      render: (row: any) =>
        row?.performedWithLvalue ? row.performedWithLvalue.lovDisplayVale : row.performedWithLkey
    },
    {
  ...
  const ishiharaColorBlindnessColumns = [
    ...
    },
    {
      ...
      render: (row: any) =>
        row?.deficiencyTypeLvalue ? row.deficiencyTypeLvalue.lovDisplayVale : row.deficiencyTypeLkey
    },
    {
      key: 'details',
      title: <Translate>EDIT</Translate>,
  ...
  const interpretationDiagnosisColumns = [
    ...
    },
    {
      ...
      render: (row: any) =>
        row?.followUpDate ? new Date(row.followUpDate).toLocaleDateString('en-GB') : ' '
    },
    {
      key: 'recommendations',
  ...
  const tabData = [
    {
      ...
      content: (
        <MyTable
          ...
          onRowClick={rowData => {
            setOptometricExam({ ...rowData });
            rowData?.icdCode?.icdCode && rowData?.icdCode?.description
              ? setSelectedIcd10({
                  text: `${rowData.icdCode.icdCode} - ${rowData.icdCode.description}`
                })
              : null;
            rowData?.icdCode2?.icdCode && rowData?.icdCode2?.description
              ? setSecondSelectedicd10({
                  text: `${rowData.icdCode2.icdCode} - ${rowData.icdCode2.description}`
                })
              : null;
