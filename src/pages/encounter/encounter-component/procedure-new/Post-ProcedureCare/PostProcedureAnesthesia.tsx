...
const PostProcedureAnesthesia = forwardRef<anesthsiaRef, AnesthesiaProps>(
  ({ procedure, user, ...props }, ref) => {
    ...
    useEffect(() => {
      const oxsatscore =
        oxsatQueryResponse?.object?.find(item => item.key === anesthesia?.oxygenSaturationLkey)
          ?.score || 0;
      const conscscore =
        conscLovQueryResponse?.object?.find(item => item.key === anesthesia?.consciousnessLkey)
          ?.score || 0;
      const circuscore =
        circuLovQueryResponse?.object?.find(item => item.key === anesthesia?.circulationLkey)
          ?.score || 0;
      const respirscore =
        respirLovQueryResponse?.object?.find(item => item.key === anesthesia?.respirationLkey)
          ?.score || 0;
      const activityscore =
        activityLovQueryResponse?.object?.find(item => item.key === anesthesia?.activityLkey)
          ?.score || 0;
    ...
    return (
      <SectionContainer
        title={<Text>Post-Procedure Anesthesia</Text>}
        content={
