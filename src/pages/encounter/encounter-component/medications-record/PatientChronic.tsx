...
const PatientChronic = ({ genericMedicationListResponse, customeInstructions, patient }) => {
  ...
  const { data: genericMedicationActiveIngredientListResponseData, refetch: refetchGenric } =
  ...
  const tableColumns = [
    ...
    },
    {
      ...
      flexGrow: 1,
      render: (rowData: any) => {
        return genericMedicationListResponse?.find(
    ...
    },
    {
      ...
      render: (rowData: any) => {
        return genericMedicationListResponse?.find(
          item => item.key === rowData.genericMedicationsKey
        )?.dosageFormLvalue.lovDisplayVale;
      }
    },
    {
      key: '',
      ...
      render: (rowData: any) => {
        if (rowData.sourceName == 'Prescription') {
          ...
          if (rowData.instructionsTypeLkey === '3010606785535008') {
            return (
              customeInstructions?.find(item => item.prescriptionMedicationsKey === rowData.key)
                ?.dose +
              ',' +
              customeInstructions?.find(item => item.prescriptionMedicationsKey === rowData.key)
                ?.roaLvalue?.lovDisplayVale +
              ',' +
              customeInstructions?.find(item => item.prescriptionMedicationsKey === rowData.key)
